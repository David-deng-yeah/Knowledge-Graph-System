...
def routers_insert(request):
    ...
    sql = "insert into web.tmp (self_key, srt, parent_key, path, component, title, url, db, sql_sentence) values ('{self_key}', '{srt}', '{parent_key}', '{path}', '{component}', '{title}', '{url}', '{db}', '{sql_sentence}')".format(self_key=self_key, srt=srt, parent_key=parent_key, path=path, component=component, title=title, url=url, db=db, sql_sentence=sql_sentence)
    rows,cols,rt = db1.dosql(sql)
    #
    af = rt.rowcount
    sts = 'error'
...
def bgd_search(request):
    ...
    sql = "select * from database_1.bgds_inspection_details_2020_view where 报关单号={报关单号} limit 1".format(报关单号=报关单号)
    rows,cols,rt = db1.dosql(sql)
    sql_data = []
    for row in rows:
        row_dict = {}
