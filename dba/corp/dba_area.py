...
sql = """
select `AREA_CODE` AS `CODE`,`AREA_NAME` AS `NAME`,'AREA_I' as SRC from app_db_cbec_zs.cbec_elist where AREA_NAME is not null group by AREA_CODE,AREA_NAME
UNION
select `AREA_CODE` AS `CODE`,`AREA_NAME` AS `NAME`,'AREA_E' as SRC from app_db_cbee_zs.cbee_elist where AREA_NAME is not null group by AREA_CODE,AREA_NAME
"""
import pandas as pd
...
# todo get field from gbasecols...(when this script is more generic...)
#dosql("""
