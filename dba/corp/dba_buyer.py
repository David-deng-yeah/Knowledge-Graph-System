...
#select `OWNER_CODE` AS `CODE`,`OWNER_NAME` AS `NAME`,'OWNER_I' as SRC from app_db_cbec_zs.cbec_elist where OWNER_NAME is not null group by OWNER_CODE,OWNER_NAME
#UNION
#select `OWNER_CODE` AS `CODE`,`OWNER_NAME` AS `NAME`,'ONWER_E' as SRC from app_db_cbee_zs.cbee_elist where OWNER_NAME is not null group by OWNER_CODE,OWNER_NAME
sql = """
#select `BUYER_CODE` AS `CODE`,`BUYER_NAME` AS `NAME`,'BUYER_I' as SRC from app_db_cbec_zs.cbec_elist where BUYER_NAME is not null group by BUYER_CODE,BUYER_NAME
"""
import pandas as pd
...
print("CORP(I+E):")
#select COUNT(*) C, GROUP_CONCAT(code) grp_code, GROUP_CONCAT(`SRC`) grp_src, `name` from o_corp group by `name` having c>1 ORDER BY c DESC;
df = pd.read_sql("""
        select COUNT(*) C, `name`, GROUP_CONCAT(`SRC`) grp_src from o_corp group by `name` having c>1 ORDER BY c DESC;
        """, con=dbtgt.engine);
print(df)
